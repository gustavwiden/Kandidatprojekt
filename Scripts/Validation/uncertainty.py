...
bound_factors = [1.2, 1.2, 1.000001, 1.000001, 1.000001, 1.000001, 1.000001, 1.000001, 1.000001, 1.000001, 1.1, 1.000001, 2, 1.000001, 2, 1.000001, 5, 1.000001]
...
def callback_log(x, file_name='PK-test-temp'):
...
def callback_evolution_log(x,convergence):
    callback_log(x, file_name='PK-test-temp-evolution')
...
def proxy_f_joint(x):
    return fcost_joint(np.exp(x), model_sims, PK_data, PD_data)[0]

def proxy_f_joint_with_save(x):
    return fcost_uncertainty(x, model_sims, PK_data, PD_data)

# Define pyPESTO objective for optimization (optional)
custom_objective = pypesto.Objective(fun=proxy_f_joint)
problem = pypesto.Problem(
    objective=custom_objective,
    lb=bounds_log.lb,
    ub=bounds_log.ub,
    x_guesses=[initial_params_log],
    x_scales=['log'] * len(initial_params_log),
)

# OPTIONAL: Run optimization (can skip if already optimized with DE)
# optimizer = optimize.ScipyOptimizer()
# result = optimize.minimize(problem=problem, optimizer=optimizer, n_starts=5)

# Use best found param (from DE or PESTO) for MCMC starting point
if best_param is not None:
    x0_for_mcmc = np.log(best_param)
else:
    x0_for_mcmc = initial_params_log


# Define new objective for sampling that saves acceptable sets
objective_sampling = pypesto.Objective(fun=proxy_f_joint_with_save)
problem_sampling = pypesto.Problem(
    objective=objective_sampling,
    lb=bounds_log.lb,
    ub=bounds_log.ub,
    x_guesses=[x0_for_mcmc],
    x_scales=['log'] * len(initial_params_log),
)

# Run MCMC sampling using Adaptive Metropolis
sampler = sample.AdaptiveMetropolisSampler()
result_sampling = sample.sample(
    problem=problem_sampling,
    sampler=sampler,
    n_samples=100000,
)

# Extract and plot results (example for 1st parameter)
import matplotlib.pyplot as plt
trace_x = result_sampling.sample_result["trace_x"]
trace_array = trace_x[0]

# Plot posterior distributions
fig, axs = plt.subplots(3, 6, figsize=(18, 9))
axs = axs.flatten()

for i in range(min(len(trace_array[0]), len(axs))):
    axs[i].hist(trace_array[:, i], bins='auto', edgecolor='black')
    axs[i].set_title(f'Param {i}')

plt.tight_layout()
plt.show()
# === PYPESTO MCMC SECTION END ===
