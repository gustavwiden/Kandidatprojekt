...
data = load_data('HV_PK_data', 'HV_PD_data', 'SLE_PK_data', 'SLE_PD_data', 'SLE_CLE_PK_validation_data', 'HV_vs_SLE_plasma_PK_response_data', 'HV_vs_SLE_plasma_PD_response_data', 'SLE_IV_dose_skin_PD_response_data')
...
def plot_HV_PD_simulations(final_params, acceptable_params, sims, PD_data, subset_doses=None):
    ...
    for dose in all_keys:
        ...
        max_t = PD_data[dose]["time"][-1] if dose in PD_data else 2688
        time_vector = np.arange(-400, max_t + 5000, 1)
        time_weeks = time_vector / 168.0
...
def plot_HV_PD_simulations_comparison(final_params, acceptable_params, sims, PD_data, selected_doses):
    ...
    fig_bar, ax_bar = plt.subplots(figsize=(10, max(2, len(bar_labels) * 0.6)), layout='constrained')
...
def plot_HV_vs_SLE_PD_simulations(HV_final_params, SLE_final_params, acceptable_params, HV_sims, SLE_sims, HV_PD_data, SLE_PD_data):
    ...
    for i, (HV_dose, label) in enumerate(zip(HV_sims.keys(), labels)):
        ...
        max_t = HV_PD_data[HV_dose]["time"][-1] if HV_dose in HV_PD_data else 2688
        time_vector = np.arange(-400, max_t + 5000, 1)
        time_weeks = time_vector / 168.0
...
def plot_loading_phase_clinical_menu(params, models, density_labels):
    ...
    for density_label in density_labels:
        ...
        rect = Rectangle((dose_idx_1 - 0.5, row_idx - 0.5), 1, 1, fill=False, edgecolor='black', linewidth=5, zorder=5, clip_on=False)
        ax1.add_patch(rect)
...
def plot_skin_plasma_concentration_ratio(params, models, time_vector):
    ...
    # for i, (label, (m_key, p_suffix)) in enumerate(HV_scenarios.items()):
        ...
        # This does not necessarily need to be changed when switching between SLE and HV, but using
        # target_skin_conc for SLE and target_plasma_conc for HV gives a more accurate measurement of the
        # what the skin-plasma ratio is just before the simulation lines are cut off by the y- or x-axis
